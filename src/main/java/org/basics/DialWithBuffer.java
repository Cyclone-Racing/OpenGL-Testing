...
public class DialWithBuffer implements GLEventListener {
    ...
    public void init(GLAutoDrawable glAutoDrawable) {
        ...
        final float radius = 2 * 0.45f;

        float originX = 0;
        float originY = radius / -2f;

        vertices.put(originX);
        vertices.put(originY);

        float increment = ((float) Math.PI / (sampleCount - 1));
        for (double angle = 0; angle < Math.PI; angle += increment) {
            vertices.put(-1 * (originX + ((float) Math.cos(angle) * radius)));
            vertices.put(originY + ((float)Math.sin(angle) * radius));
        }
...
//            value = (float) (Math.random() * (maxValue - minValue)) + minValue;
            ...
            gl.glDrawArrays(GL2.GL_TRIANGLE_FAN, 0, (int) (sampleCount * percent) /* DrawCount Goes Here! */);
